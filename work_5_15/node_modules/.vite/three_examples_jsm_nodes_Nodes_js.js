import {
  ...
  Vector4,
  sRGBEncoding
} from "./chunk-FTXZG6AJ.js";
import {
  __export,
  __publicField
} from "./chunk-SNSY3PSS.js";
...
__publicField(MathNode, "RECIPROCAL", "reciprocal");
...
var _NormalNode = class extends Node_default {
  ...
  generate(builder) {
    ...
    if (scope === _NormalNode.GEOMETRY) {
      ...
      const vertexNode = new OperatorNode_default("*", new ModelNode_default(ModelNode_default.NORMAL_MATRIX), new _NormalNode(_NormalNode.LOCAL));
      ...
      const vertexNode = new MathNode_default(MathNode_default.TRANSFORM_DIRECTION, new _NormalNode(_NormalNode.VIEW), new CameraNode_default(CameraNode_default.VIEW_MATRIX));
      outputNode = new MathNode_default(MathNode_default.NORMALIZE, new VaryingNode_default(vertexNode));
    }
...
var _TangentNode = class extends Node_default {
  ...
  generate(builder) {
    ...
    if (scope === _TangentNode.GEOMETRY) {
      ...
      outputNode = new VaryingNode_default(new SplitNode_default(new _TangentNode(_TangentNode.GEOMETRY), "xyz"));
    } else if (scope === _TangentNode.VIEW) {
      const vertexNode = new SplitNode_default(new OperatorNode_default("*", new ModelNode_default(ModelNode_default.VIEW_MATRIX), new _TangentNode(_TangentNode.LOCAL)), "xyz");
      outputNode = new MathNode_default(MathNode_default.NORMALIZE, new VaryingNode_default(vertexNode));
    } else if (scope === _TangentNode.WORLD) {
      const vertexNode = new MathNode_default(MathNode_default.TRANSFORM_DIRECTION, new _TangentNode(_TangentNode.VIEW), new CameraNode_default(CameraNode_default.VIEW_MATRIX));
      outputNode = new MathNode_default(MathNode_default.NORMALIZE, new VaryingNode_default(vertexNode));
    }
    return outputNode.build(builder, this.getNodeType(builder));
  }
...
var TangentNode = _TangentNode;
__publicField(TangentNode, "GEOMETRY", "geometry");
__publicField(TangentNode, "LOCAL", "local");
__publicField(TangentNode, "VIEW", "view");
__publicField(TangentNode, "WORLD", "world");
var TangentNode_default = TangentNode;

// node_modules/three/examples/jsm/nodes/accessors/BitangentNode.js
var _BitangentNode = class extends Node_default {
...
var BitangentNode = _BitangentNode;
__publicField(BitangentNode, "GEOMETRY", "geometry");
__publicField(BitangentNode, "LOCAL", "local");
__publicField(BitangentNode, "VIEW", "view");
__publicField(BitangentNode, "WORLD", "world");
...
var _PositionNode = class extends Node_default {
  ...
  generate(builder) {
    ...
    if (scope === _PositionNode.GEOMETRY) {
      ...
      const vertexPositionNode = new MathNode_default(MathNode_default.NEGATE, new _PositionNode(_PositionNode.WORLD));
      outputNode = new MathNode_default(MathNode_default.NORMALIZE, new VaryingNode_default(vertexPositionNode));
    }
    return outputNode.build(builder, this.getNodeType(builder));
  }
...
__publicField(PositionNode, "WORLD_DIRECTION", "worldDirection");
...
var MaxMipLevelNode = class extends UniformNode_default {
  ...
  update() {
    ...
    const image = images && images.length > 0 ? images[0]?.image || images[0] : texture2.image;
...
var StackNode = class extends Node_default {
  ...
  assign(targetNode, sourceValue) {
    this.nodes.push(new OperatorNode_default("=", targetNode, sourceValue));
    return this;
  }
  build(builder, ...params) {
...
var func = (code2, includes3) => {
  ...
  node.call = (...params) => nodeObject(call2(params.length > 1 || params[0]?.isNode === true ? nodeArray(params) : nodeObjects(params[0])));
  return node;
};
...
var CubeTextureNode = class extends TextureNode_default {
  ...
  getDefaultUV() {
    defaultUV2 ||= new ReflectVectorNode_default();
    return defaultUV2;
  }
  generate(builder, output) {
    const { uvNode, levelNode } = builder.getNodeProperties(this);
...
var _BlendModeNode = class extends Node_default {
...
var BlendModeNode = _BlendModeNode;
__publicField(BlendModeNode, "BURN", "burn");
__publicField(BlendModeNode, "DODGE", "dodge");
__publicField(BlendModeNode, "SCREEN", "screen");
__publicField(BlendModeNode, "OVERLAY", "overlay");
...
var _ColorAdjustmentNode = class extends TempNode_default {
...
__publicField(ColorAdjustmentNode, "SATURATION", "saturation");
__publicField(ColorAdjustmentNode, "VIBRANCE", "vibrance");
__publicField(ColorAdjustmentNode, "HUE", "hue");
...
var _ColorSpaceNode = class extends Node_default {
...
var resolution;
var _ViewportNode = class extends Node_default {
  constructor(scope) {
    super();
    this.scope = scope;
...
var ViewportNode = _ViewportNode;
__publicField(ViewportNode, "COORDINATE", "coordinate");
__publicField(ViewportNode, "RESOLUTION", "resolution");
__publicField(ViewportNode, "TOP_LEFT", "topLeft");
__publicField(ViewportNode, "BOTTOM_LEFT", "bottomLeft");
__publicField(ViewportNode, "TOP_RIGHT", "topRight");
__publicField(ViewportNode, "BOTTOM_RIGHT", "bottomRight");
...
var LightingContextNode = class extends ContextNode_default {
  ...
  construct(builder) {
    ...
    if (lightingModelNode?.indirectDiffuse)
      lightingModelNode.indirectDiffuse.call(context2);
    if (lightingModelNode?.indirectSpecular)
      lightingModelNode.indirectSpecular.call(context2);
    if (lightingModelNode?.ambientOcclusion)
...
var _TimerNode = class extends UniformNode_default {
...
var RotateUVNode = class extends TempNode_default {
  ...
  construct() {
    ...
    return vec2(add(add(mul(cosAngle, sub(uvNode.x, centerNode.x)), mul(sinAngle, sub(uvNode.y, centerNode.y))), centerNode.x), add(sub(mul(cosAngle, sub(uvNode.y, centerNode.y)), mul(sinAngle, sub(uvNode.x, centerNode.x))), centerNode.y));
  }
};
...
var getDistanceAttenuation = new ShaderNode((inputs) => {
  ...
  return cond(greaterThan(cutoffDistance, 0), mul(distanceFalloff, pow2(clamp(sub(1, pow4(div(lightDistance, cutoffDistance)))))), distanceFalloff);
});
...
var exp2Fog = nodeProxy(FogExp2Node_default);
...
var NodeBuilder = class {
  ...
  hasGeometryAttribute(name) {
    return this.geometry?.getAttribute(name) !== void 0;
  }
...
var getSpotAttenuation = (coneCosine, penumbraCosine, angleCosine) => smoothstep(coneCosine, penumbraCosine, angleCosine);
...
var NodeMaterial = class extends ShaderMaterial {
  ...
  generateOutput(builder, { diffuseColorNode, outgoingLightNode }) {
    ...
    if (fogNode?.isNode !== true && builder.scene.fog) {
      ...
      if (fog2.isFogExp2) {
        fogNode = exp2Fog(reference("color", "color", fog2), reference("density", "float", fog2));
      } else if (fog2.isFog) {
  ...
  setDefaultValues(values) {
    for (const property2 in values) {
      ...
      if (this[property2] === void 0) {
        this[property2] = value?.clone?.() || value;
      }
    }
...
//# sourceMappingURL=three_examples_jsm_nodes_Nodes_js.js.map
